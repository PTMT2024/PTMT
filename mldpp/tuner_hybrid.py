...
class TunerHybrid:
    def __init__(self, workload, memmanagement, bench, logdir, collectperiod):
        ...
        self.knn_predictor = PredictorV2(self.__knn_model_name(), True)
    ...
    def __rl_model_name(self):
        ...
        for suffix in suffixes:
            if workload_name.endswith(suffix):
                ...
                break
        # if self.memmanagement == "autonuma" or self.memmanagement == "tiering08":
        #     return f"model.{workload_name}.{self.memmanagement}.rl_model.ipc.10s"
        return f"model.{workload_name}.{self.memmanagement}.rl_model.ipc.{self.collect_period}s"

    def __knn_model_name(self):
        suffixes = ['_l', '_s', '_x']
        workload_name = self.workload
        for suffix in suffixes:
            if workload_name.endswith(suffix):
                workload_name = workload_name.split('_')[0]  # Keep only the part before the first '_'
                break
        return f"model.{workload_name}.{self.memmanagement}.knn_model.ipc.v2.no_traffic_norm.{self.collect_period}s.pkl.dis_thres.pkl"
    ...
    def __update_mm_config_knn(self, bench_pid, model_name, new_datapoint):
        return self.memmanagement_module.update_ml(self.knn_predictor, bench_pid, model_name, new_datapoint, 'ipc')
    ...
    def __check_outlier(self, count):
        if count == 1:
            return False
        # TODO
        return False
    
    def __run_and_collect_rl(self, count, bench_pid, ldram_size):
        self.rl_predictor.learn()

    def __run_and_collect_knn(self, count, bench_pid, ldram_size):
        random_config = {}
        if count == 1:
            random_config = self.__reset_mm_config(bench_pid) 
        else:
            random_config, info = self.__update_mm_config_knn(bench_pid, self.__knn_model_name(), self.previous_status)
        if info['is_outlier']:
        ...
        print(f"__run_and_collect_knn {count} times")
    ...
    def do(self):
        ...
        subprocess.Popen(self.start_workload_cmd, shell=True)
        ...
        collect_metrics_cmd = f"/home/sherry/projects/sk_prokect/mldpp/collector/mldpp_metrics_collector 0 {bench_pid} {self.collect_period} {pebs_target_pid} {self.logdir}"
        ...
        while True:
            ...
            if self.is_outlier:
                self.rl_predictor = RLPredictor(self.__rl_model_name(), self.memmanagement, self.collect_period, self.logdir, ldram_size, bench_pid, hybrid_mode=True, count - 1)
                self.__run_and_collect_rl(count, bench_pid, ldram_size)
            else:
                self.__run_and_collect_knn(count, bench_pid, ldram_size)
