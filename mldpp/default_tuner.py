...
class DefaultTuner:
    ...
    def __default_config(self, bench_pid):
        return self.memmanagement_module.default_config(bench_pid)
    ...
    def __parse_metrics_result(self, count):
        ...
        print(f"__parse_metrics_result {count} at {datetime.datetime.now()}")
    ...
    def __run_and_collect(self, count, bench_pid, ldram_size):
        random_config = self.__default_config(bench_pid)
        if count == 1:
