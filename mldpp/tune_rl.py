...
class TunerRL:
    ...
    def __run_and_collect(self, count, bench_pid, ldram_size):
        random_config = {}
        if count == 1:
            obs, info = self.__reset_rl()
            # self.__save_existing_result(count,  info["config"], ldram_size, info["metrics_result"])
        else:
            obs, reward,  terminated, truncated, info= self.__update_mm_config_rl()
            # self.__save_existing_result(count,  info["config"], ldram_size, info["metrics_result"])
    ...
    def do(self):
        ...
        sleep(4)
        self.predictor = RLPredictor(self.__model_name(), self.memmanagement, self.collect_period, self.logdir, ldram_size, bench_pid)
        count = 0
        while True:
            # try:
            #     os.kill(bench_pid, 0)
