...
class TunerKnnSensitivity:
    def __init__(self, workload, memmanagement, bench, logdir, collectperiod, k_value):
        ...
        self.predictor = PredictorV2(self.__model_name())
    ...
    def __model_name(self):
        ...
        if self.memmanagement == "mldpp":
            return f"model.{workload_name}.{self.memmanagement}.knn_model.k{self.k_value}.10s.pkl"
        elif self.memmanagement == "tpp":
            return f"model.{workload_name}.{self.memmanagement}.knn_model.k{self.k_value}.10s.pkl"
        else:
            return f"model.{workload_name}.{self.memmanagement}.knn_model.k{self.k_value}.10s.pkl"
    ...
    def __update_mm_config_ml(self, bench_pid, model_name, new_datapoint):
        return self.memmanagement_module.update_ml(self.predictor, bench_pid, model_name, new_datapoint, 'ipc')
    ...
    def __run_and_collect(self, count, bench_pid, ldram_size):
        ...
        if count == 1:
            ...
            random_config, info = self.__update_mm_config_ml(bench_pid, self.__model_name(), self.previous_status)
